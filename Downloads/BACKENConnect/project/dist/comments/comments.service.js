...
let CommentsService = class CommentsService {
    ...
    async create(testimonyId, userId, createCommentDto) {
        const testimony = await this.prisma.testimony.findUnique({
            where: { id: testimonyId },
        });
        if (!testimony) {
            throw new common_1.NotFoundException(`Testimony #${testimonyId} not found`);
        }
        if (testimony.userId === userId) {
            throw new common_1.ForbiddenException('You cannot comment on your own testimony');
        }
    ...
    async findAllByTestimony(testimonyId) {
        const testimony = await this.prisma.testimony.findUnique({
            where: { id: testimonyId },
        });
        if (!testimony) {
